...
本项目与仓库根目录的 `CRIS/.venv` 共用一份 Python 虚拟环境。说明与配置都在本目录：[`README.md`](README.md)、[`agent.md`](agent.md)、[`config/settings.yaml`](config/settings.yaml)。子项目之间的关系见仓库根目录 [README.md](../README.md)。

单对比较和批量比较都从同一个入口启动：`main.py`。
...
| 结构与关键元数据 | 晶格参数、原子数、元素、周期分数坐标、占据率、磁矩、CIF 里声明的空间群；不忽略行序时还比较 ASU 原子标签 |
...
**不要改的：** 为迁就路径去改程序常量。应对齐文件名，或改 yaml 里的 `compare` 段，而不是在入口里再传一套路径。
...
菜单**不再询问文件或目录路径**。晶格 / 分数坐标 / 占据率与磁矩容差默认取 `config/settings.yaml` 的 `defaults`（当前均为 `1e-5`）。
...
| **y** | 不按行号，按「同种元素 + 分数坐标足够接近」做完整二分匹配。同一套原子只是写成不同顺序时（例如 `Eu, Al, Al` 对 `Al, Eu, Al`）可以选 y。实现使用增广路径而非逐行贪心，因此一个宽松候选不会抢占另一个原子的唯一可行配对。这**不会**忽略坐标、元素种类或展开后位点占据率的真实差异；ASU 行标签只是显示标识，启用此选项时不作为失败条件。原始磁矩循环只有在能与展开位点一一映射时才可比较。 |
...
| `--ignore-atom-order` | 关 | 忽略 CIF 原子**行顺序**，按「元素 + 分数坐标」配对；占据率按对称展开后的位点比较，ASU 标签不判失败；默认仍按行号对应 |
...
并把经精确整幺模变换证明等价的 basis 统一到官网设置后比较，使用上游只读
审计器：
