...
- 畴数 == 子群指数；
- **物理自洽性自检**（`isocore/utils/self_check.py`）：零振幅回退、子群规则、
  模式正交性、振幅线性、对称性守恒；
- 官网参考比对：EuAl4 → LD1（参数 k 点，官网 g=1/6 ↔ iso 参数 a=1/12）→
  P4mm #99 的子群路径与官网导出的 `LD1_C1_subgroup.cif` 头部**完全一致**
  （IR LD1、OPD C1、s=12、i=24、c 轴 6 倍超胞）。该参考文件为 ISODISTORT
  导出的**零振幅框架文件**（序参量全 0），参数 k 点的模式计算为已知限制
  （见差异 5），因此只比对子群路径而非坐标。

已知边界：
- 1 原子原胞的退化结构（如单原子 bcc/fcc）在 Γ 点的极性模式是刚性平移，
  纯位移畸变无法降低对称性（需应变耦合），验证中归类为
  “degenerate-rigid-translation”，不属于算法错误；
- 多物种共享同一 Wyckoff 字母的结构（如 Fe/O 同在一般位置）归类为
  “limitation-shared-letter”，见已知差异 11。

实现说明（超胞基矢的分数矩阵）：
- iso 的子群基矢以母相**惯用晶胞**为单位给出；对带心母相（F/I/C 心），
  子群胞可能只是惯用胞的**亚胞**（如 Fm-3m 4 原子 → I4/mmm 2 原子，
  基矢行列式 0.5）。pymatgen 的 `Structure * matrix` 只接受整数矩阵，
  直接传入会抛 `LinAlgError: Singular matrix`。
- `isocore/structure/coordinate_transform.py` 的 `build_supercell` 现可处理
  分数基矢：新格子 = 基矢 × 母相格子，原子映射进新格子后按周期等价合并
  重合位点（位移畸变自然分离重合位点，不会误并）。

最近实测（2026-08，32 个母相）：
- 加载识别 32/32、Method 1 候选数/模式数终端与 API 全一致 32/32；
- 畸变结构 spglib 对称性 == 目标子群 28/32；
- 退化结构（1 原子原胞刚性平移）2 个：Fm-3m、Fd-3m；
- 共享 Wyckoff 字母限制 2 个：P4/nmm、R-3c；
- 官网参考 LD1 → P4mm #99 路径比对 path_match=True（IR LD1、OPD C1、
  s=12、i=24、c 轴 6 倍超胞，参考 CIF 为零振幅框架文件）；
- 未通过 0、流程错误 0。

注 1：相比上一轮 26/32，Pm-3m 由“退化”转为“通过”——其单原子原胞选中
区边界 R₄⁻ 模式（k=(1/2,1/2,1/2)，2×2×2 超胞），Bloch 相位调制
（cos(2πk·t) 逐副本）使一半副本反向位移，畸变结构正确降为子群对称性。

注 2：相比 27/32，Im-3m 由“退化”转为“通过”——修复了**带心母相区边界
模式的映射器相位丢失**（见“已知差异”第 14 条）：BUSH 对 H4- 输出两个
相位反号的代表点，旧映射按“模格点等价”首个代表胜出，把带心副本的位移
错误地取成相同（退化为刚性平移）；现按“周期等价（mod 1）”把代表点
分配给对应原子，H4- 三个子群（P4_2/nmc #129 / Cmcm #63 / R-3m #166）
均正确降对称。回归测试：`test_im3m_h4_minus_zone_boundary_phase`。

### 科研级扩展验证（金标准 / 三接口 / 鲁棒性 / 可重复性）

在 30-CIF 全流程之上，`tests/` 提供四层补充验证（对应科研级测试方案）：

1. **金标准回归**（`tests/test_golden_standard.py`，依赖 WSL）：
   - 以经典钙钛矿 SrTiO₃（Pm-3m #221，位点对齐官网示例）为金标准母相，
     验证三条文献公认相变路径：**R₄⁺ 倾转 → I4/mcm #140**、
     **M₃⁺ 同相倾转 → P4/mbm #127**、**Γ₄⁻ 极性 → P4mm #99**；
   - `StructureMatcher(ltol=1e-5, stol=1e-3, angle_tol=0.001)` 语义比对
     （自动处理原子顺序/对称等价），零振幅回退、振幅线性（相对误差 ≤1e-4）、
     官网 LD1 参考 CIF 原子数/元素/空间群比对。
2. **三接口一致性**（`tests/test_three_interface.py`，依赖 WSL）：
   同源输入分别走 Python API 与网页 HTTP 端点，method1_options /
   候选数 / 模式数 / 生成畸变空间群完全一致。
3. **多模式叠加**（`tests/test_multi_mode.py`，依赖 WSL）：
   同时激活 2~3 个 IR 的混合畸变 = 各模式位移线性叠加（≤1e-6），
   且混合结构对称性仍 == 目标子群。
4. **格式兼容 / 异常输入 / 可重复性**（`tests/test_format_compat.py`、
   `tests/test_robustness_extended.py`，无需 WSL）：
   - CIF / VASP POSCAR / xyz 输入（`read_structure` 按扩展名识别；
     `load_structure` 直接接受）；
   - 非法输入（NaN/Inf 位移、损坏文件、未知格式、极大/极小振幅）明确报错
     或正确计算，绝不静默输出错误结构；
   - 同一输入两次运行逐字节一致（确定性）。

实现说明（非零 k 点的 Bloch 相位）：
- 子群在非 Γ k 点（如 R/M/X 点）时，超胞各副本的位移必须按
  **Bloch 相位 cos(2πk·t)** 调制（t 为副本相对母相原胞的平移）。
  此前引擎对所有副本施加相同位移（仅 Γ 点正确），导致如 SrTiO₃ M₃⁺
  本应得 P4/mbm #127 却得到 #38 的静默错误结果。
- `distortion_engine.generate_single_mode / generate_modes` 现接受
  `k_vector`（`search_method_2` 自动从 k 点标签解析），逐副本调制；
  `generate_modes` 同时修正了“母相分数位移直接加到超胞坐标”的
  坐标系变换（Δf_sc = Δf_parent @ inv(basis)）。
...
11. **多物种共享 Wyckoff 字母时的模式映射（已知限制）**：当结构中多个物种
    落在同一个 Wyckoff 位置（如 Fe 与 O 同在一般位置 16k），iso 的 DISPLAY
    BUSH 只输出一份符号化代表点（取位约定与结构实际坐标不一致，无法按位置
    解析到原子），本地映射器无法正确分配位移。30-CIF 验证中此类结构
    （P4/nmm、R-3c 双物种同一般位置）被单独归类为
    `limitation-shared-letter`，**不会静默输出错误结果**。
    规避：此类结构请改用**不同 Wyckoff 位置**的母相模型，或结合官网输出。
12. **多维模式的生成方向（v1 近似）**：官网按 OPD 方向确定多维 IR 各分量的
    权重；本地生成时取各分量**等权求和**再归一化（OPD 通用方向近似），
    对全对称（GM1+）模式与 1D 模式结果正确，特定 OPD 方向的多维模式
    方向可能与官网略有差异。
13. **物理自洽性自检（内置防错）**：`isocore/utils/self_check.py` 提供 5 项
    无需金标准的自检（零振幅回退、子群规则、模式正交性、振幅线性、
    对称性守恒），任一失败即说明计算逻辑存在 bug；30-CIF 验证与
    `tests/test_self_check.py` 均会运行这些自检。
14. **带心母相区边界模式的相位映射（已修复 + 残留边界）**：
    - **已修复**：非 Γ k 点模式下，BUSH 会为带心副本输出相位反号的独立
      代表点（如 Im-3m H4-：`(0,0,0)→(1,0,1)` 与 `(-1/2,1/2,1/2)→(-1,0,-1)`，
      两点相差原始格点平移、互为“模格点等价”）。旧映射器按“首个代表
      胜出”把全部原子分给同一代表，相位丢失、畸变退化为刚性平移
      （曾导致 Im-3m H4- 全部子群对称性校验失败）。现改为优先按
      “周期等价（mod 1）”把代表点分配给对应原子，再回退模格点等价；
      Im-3m 三个 H4- 子群（#129/#63/#166）均正确降对称。
    - **残留边界（fcc 带心，如 Fm-3m/Fd-3m 的 L 点模式）**：子群超胞为
      奇异分数基矢（行列式 0.5/0.75 等亚胞）时，区边界模式的生成结果
      与目标子群不一致（实测 L2-/L1+ 模式畸变对称性偏高或错误）。此类
      场景不在 30-CIF 主流程的采样范围内（采样仅取前 12 个可算模式，
      这些母相首轮即命中 Γ 刚性平移模式），暂归类为已知边界，后续版本
      按“超胞副本平移须取分数基矢对应的格点平移”思路修复。
